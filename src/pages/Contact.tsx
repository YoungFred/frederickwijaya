    ...
    <Layout showEchelonFooter>
      <section className="container-wide min-h-[calc(100vh-200px)] py-16 md:py-24">
        <div className="grid grid-cols-1 gap-16 lg:grid-cols-[1.1fr_0.9fr] lg:gap-24">
          <div>
            <p className="text-label mb-6 text-accent">{t.contact.kicker}</p>
            <h1 className="text-display max-w-3xl">{t.contact.title}</h1>
            <p className="mt-8 max-w-xl text-xl leading-relaxed text-muted-foreground">{t.contact.intro}</p>
          ...
          <div className="flex flex-col justify-end border-t border-separator pt-8 lg:border-l lg:border-t-0 lg:pl-12 lg:pt-0">
            <p className="text-label mb-4">{t.common.basedIn}</p>
            ...
            <p className="mt-6 max-w-sm text-sm leading-relaxed text-muted-foreground">{t.contact.outro}</p>
          </div>
        </div>
      </section>
    </Layout>
